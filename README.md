...
> PostgreSQL (`:15329`), Redis (`:16192`), Kafka (`:19043`) 시스템이 백그라운드에서 실행됩니다. Docker Compose는 기본적으로 `.env`를 읽기 때문에, 개발 실행은 반드시 `--env-file .env.dev` 또는 아래 래퍼 스크립트를 사용해야 합니다.
...
| 파일        | 용도                                                                          | Git 추적     |
| ----------- | ----------------------------------------------------------------------------- | ------------ |
| `.env.dev`  | 로컬 개발용 포트 및 DB 세팅 (5자리 단위 포트 포워딩) | ❌ gitignore |
...
| 명령어                                        | 실행 대상                           | 용도                              |
| --------------------------------------------- | ----------------------------------- | --------------------------------- |
| `docker compose --env-file .env.dev up -d`                        | 인프라 기본 (DB, Redis, Kafka)      | 로컬 개발용 인프라만 구성         |
