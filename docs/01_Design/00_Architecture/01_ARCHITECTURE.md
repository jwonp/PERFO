...
1. [문서 범위](#1-문서-범위)
2. [현재 실제 아키텍처](#2-현재-실제-아키텍처)
3. [인프라 개요](#3-인프라-개요)
4. [기술 선택 이유](#4-기술-선택-이유)
5. [배포 전략](#5-배포-전략)
6. [스케일링 전략](#6-스케일링-전략)
7. [단계별 다중화 전략](#7-단계별-다중화-전략)
8. [데이터 저장소 전략](#8-데이터-저장소-전략)
9. [관측성 전략](#9-관측성-전략)
10. [장애 복구 전략](#10-장애-복구-전략)
11. [CI/CD 및 배포 자동화](#11-cicd-및-배포-자동화)
12. [IaC 전략](#12-iac-전략)
13. [카나리 배포 운영 원칙](#13-카나리-배포-운영-원칙)
14. [보안과 운영](#14-보안과-운영)
15. [용량 계획](#15-용량-계획)
16. [관련 문서](#16-관련-문서)
...
2026-05-20 기준 실제 구현과 기본 실행 방식은 아래와 같다.

- 저장소는 `frontend/`와 `backend/`를 함께 두는 모노레포다.
- 사용자 진입점은 `frontend`의 Next.js App Router다.
- 브라우저는 backend를 직접 호출하지 않고, 대부분 `frontend/app/api/**` BFF를 통해 진입한다.
- `frontend`는 UI만이 아니라 `NextAuth` 세션, 내부 JWT 발급, 알림/푸시 저장도 담당한다.
- `backend`는 Spring Boot + Kotlin 기반 도메인 API이며, 실제 상태 변경과 트랜잭션을 소유한다.
- 데이터 저장은 주로 PostgreSQL을 사용한다.
- 프로필/티켓 이미지는 MinIO에 저장한다.
- Kafka는 티켓팅 outbox/projection 비동기 경로에 연결되어 있다.
- Redis는 compose에는 포함되지만 현재 핵심 사용자 흐름에서 비중은 낮다.

### 2.1 현재 실제 인프라 구조도

현재 기본 실행 인프라 구조는 `docker-compose.yml` 기준으로 아래와 같다.
...
flowchart TB
    ...
    subgraph Runtime["Docker Compose Runtime"]
        FE[frontend / frontend-dev<br/>Next.js + BFF]
        BE[backend / backend-dev<br/>Spring Boot API]
        PG[(postgres)]
        RD[(redis)]
        ZK[zookeeper]
        KF[kafka]
...
### 2.2 현재 실제 애플리케이션 연결 구조

현재 기본 실행 토폴로지는 다음에 가깝다.
...
flowchart LR
    ...
    BE --> KF
    BE --> MI
    FE -. optional/limited .-> RD
```

### 2.3 현재 배포 기준

- 앱: `frontend`, `frontend-dev`, `backend`, `backend-dev`
- 저장소/브로커: `postgres`, `redis`, `zookeeper`, `kafka`, `minio`, `minio-init`

현재 구조를 더 자세히 보려면 `docs/ARCHITECTURE.md`를 우선 본다. 이 문서의 이후 섹션은 그 위에 놓인 목표 인프라와 확장 전략을 설명한다.
